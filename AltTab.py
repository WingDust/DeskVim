...
class AltTab:
    ...
    def list_alttab_windows(cls):
        ...
        for i in range(0,len(handles)):
            if (win32gui.IsWindowVisible(handles[i]) and win32gui.IsIconic(handles[i])):
                handles[i]={'handle':handles[i],handles[i]:True}
            else:
                handles[i]={'handle':handles[i],handles[i]:False}
                
        def exincludeTitle(handle):
                title = win32gui.GetWindowText(handles[i]['handle']) 
                print(title)
                if(title=='KeyShowView' or title=='RecBackgroundForm' or title=='dummyLayeredWnd '):
                    return True

        for i in range(0,len(handles)):
            if(not handles[i][handles[i]['handle']] ):
                if not exincludeTitle(handles[i]['handle']):
                    flag=False
                    break 
        return flag


            # title = win32gui.GetWindowText(handles[i])
            # if AltTab.is_alttab_window(handles[i]):
